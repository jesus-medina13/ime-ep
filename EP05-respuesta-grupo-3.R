...
#Definir desviación estándar de la muestra
sd_muestra <- 1/sqrt(100)

#Definir error  estándar
SE <- sd_muestra
...
cat("El nivel de significación o alfa es de ",alfa)
...
#En el caso de que la hipótesis nula es verdadera
...
g <- g + scale_x_continuous(name="Diferencia de volumen medio [lt]")

g <- g + theme_pubr()

#Colorear la región de rechazo
...
g <- g + geom_area(data=subset(data.frame(x1,y1), x1 >= Z_critico),
               aes(x=x1,y=y1),
               colour="blue",
               fill="blue",
               alpha=0.5)
...
n2 <- power.t.test(n=NULL,
                 ...
                 alternative="one.sided")$n
n2 <- ceiling(n2)
cat("La cantidad de bidones para conseguir un poder estadístico de 0.75 con un nivel de significancia de 0.01 es de ",n2)
